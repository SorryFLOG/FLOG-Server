...
lib.callback.register('qbx_truckerjob:server:spawnVehicle', function(source, model)
    ...
    -- Security check:
    -- Only allow trucks configured in config/client.lua.
    -- The client cannot send an arbitrary vehicle model.
    ...
    local plate =
        'TRUK' .. lib.string.random('1111')

    -- Spawn selected tractor
    ...
    -- Trailer spawn position.
    -- Spawn it behind the tractor so the client can attach it.
    local heading =
        math.rad(vehicleLocation.rotation)

    local trailerDistance = 12.0

    local trailerX =
        vehicleLocation.coords.x
        - math.sin(heading) * trailerDistance

    local trailerY =
        vehicleLocation.coords.y
        + math.cos(heading) * trailerDistance

    local trailerNetId, trailer =
        qbx.spawnVehicle({
            model = `trailers2`,
    ...
    return netId, plate, trailerNetId
end)
