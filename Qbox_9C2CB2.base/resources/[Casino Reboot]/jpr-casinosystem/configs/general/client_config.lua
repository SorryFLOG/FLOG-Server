...
function RemoveTargetZone(zoneName)
    if GetResourceState("ox_target") == "started" then
        exports["ox_target"]:removeZone("casinoSystem-"..zoneName)
    else
...
local function IsOxMenu()
    return Config.MenuScript == "ox" or Config.MenuScript == "ox_lib"
end

local function QbToOxOptions(qbMenu)
    local options = {}

    for _, item in ipairs(qbMenu or {}) do
        if item and not item.isMenuHeader then
            local ev, a
            
            if item.params then
                ev = item.params.event
                a  = item.params.args
            end
           
            options[#options + 1] = {
                label = item.header or 'Option',
                args = {
                    event = ev,
                    data = a
                },
...
local function OpenOxMenu(id, title, qbMenu)
    lib.registerMenu({
        id = id,
        title = title,
        position = 'top-right',
        options = QbToOxOptions(qbMenu),
    }, function(_, _, option)
        if not option or not option.event then return end
        if option.event then
            local args = option.data
...
function HitStandMenu()
    local qbMenu = {
        { id = 1, header = "Casino System", isMenuHeader = true },
        { id = 2, header = Config.Locales["39"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 1 } },
        { id = 3, header = Config.Locales["40"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 2 } },
    }

    if IsOxMenu() and GetResourceState("ox_lib") == "started" then
        OpenOxMenu('casino_hitstand_menu', 'Casino System', qbMenu)
    else
        exports[Config.MenuScript]:openMenu(qbMenu)
    end
end
...
function HitStandDoubleMenu()
    local qbMenu = {
        ...
        { id = 2, header = Config.Locales["39"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 1 } },
        { id = 3, header = Config.Locales["40"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 2 } },
        { id = 4, header = Config.Locales["42"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 3 } },
    }

    if IsOxMenu() and GetResourceState("ox_lib") == "started" then
        OpenOxMenu('casino_hitstanddouble_menu', 'Casino System', qbMenu)
    else
        exports[Config.MenuScript]:openMenu(qbMenu)
    end
end
...
function HitSplitMenu()
    local qbMenu = {
        { id = 1, header = "Casino System", isMenuHeader = true },
        { id = 2, header = Config.Locales["39"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 1 } },
        { id = 3, header = Config.Locales["43"], params = { event = 'jpr-casinosystem:client:blackjackMenu', args = 4 } },
    }

    if IsOxMenu() and GetResourceState("ox_lib") == "started" then
        OpenOxMenu('casino_hitsplit_menu', 'Casino System', qbMenu)
    else
        exports[Config.MenuScript]:openMenu(qbMenu)
    end
end
...
RegisterNetEvent('jpr-casinosystem:client:useBar', function(args)
    ...
    if args and args.args then
        for k, v in pairs(args.args.items or {}) do
    ...
    if IsOxMenu() and GetResourceState("ox_lib") == "started" then
        OpenOxMenu('casino_bar_menu', Config.Locales["42"] or 'Bar', barMenu)
    else
...
RegisterNetEvent('jpr-casinosystem:client:memberships', function()
    ...
    if Config.Memberships then
        for k, v in pairs(Config.Memberships.items or {}) do
    ...
    if IsOxMenu() and GetResourceState("ox_lib") == "started" then
        OpenOxMenu('casino_membership_menu', Config.Locales["42"] or 'Memberships', memberMenu)
    else
